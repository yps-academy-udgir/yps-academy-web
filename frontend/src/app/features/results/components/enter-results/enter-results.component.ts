...
import { StudentInfo } from '../../../classroom/models/classroom.model';
...
export class EnterResultsComponent implements OnInit {
	...
	subjects = computed((): string[] => {
		...
		return [...new Set(classroom.facultyAssignments.map((f) => f.subject as string))];
	});
	...
	allMaxMarksFilled = computed(() => {
		...
		return subjects.length > 0 && subjects.every((s) => (maxMap[s] ?? 0) > 0);
	});

	totalMaxMarks = computed(() =>
		Object.values(this.maxMarksMap()).reduce((sum, value) => sum + (value || 0), 0)
	);
	...
	loadExistingMarks(): void {
		...
		const examType = this.selectedExamType() as ExamType;
		if (!classroomId || !examType) {
			return;
		}

		this.loading.set(true);
		this.examResultService
			.getByClassroom(classroomId, examType, this.selectedMonth(), this.selectedYear())
			.subscribe({
				next: (res) => {
					...
					for (const result of res.data ?? []) {
						const sid =
							typeof result.studentId === 'object'
								? String((result.studentId as unknown as { _id: string })._id)
								: result.studentId;
	...
	submit(): void {
		if (!this.isFilterReady() || !this.allMaxMarksFilled()) {
			return;
		}
		...
		const records = this.enrolledStudents().map((student) => {
			...
			const subjectMarks: SubjectMark[] = subjects.map((subject) => ({
				subject,
				outOf: maxMap[subject],
				marksObtained: this.getMark(sid, subject),
			}));
		...
		this.examResultService.bulkSave(payload).subscribe({
			next: (res) => {
				this.notification.success(`Results saved for ${res.data?.count ?? records.length} students`);
				this.saving.set(false);
			},
			error: (err) => {
				this.notification.error(err.message || 'Failed to save results');
				this.saving.set(false);
			},
		});
	}
